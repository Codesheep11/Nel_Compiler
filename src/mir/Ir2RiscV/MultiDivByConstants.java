...
public class MultiDivByConstants {
    ...
    static class DivEliminationOptimizer {
        ...
        private static boolean expandDivWithConstantImpl(ArrayList<RiscvInstruction> instructions, Instruction.Div instr, int maxCost) {
            ...
            Operand ret = expand(plan, instructions, iter, lhs);
            if (v2 < 0) {
        ...
        private static Operand expand(ShiftDivNode plan, ArrayList<RiscvInstruction> instructions, Operand lhs) {
    ...
    public class MulEliminationOptimizer {
        ...
        private static boolean expandMulWithConstantImpl(List<Instruction> instructions, CodeGenContext codeGenCtx, ListIterator<Instruction> iter, int maxCost) {
            Instruction inst = iter.next();
            if (inst.getOpcode() != Opcode.MUL) return false;

            Operand dst = inst.getOperand(0);
            Type type = dst.getType();
            Operand lhs = inst.getOperand(1);
            Operand rhs = inst.getOperand(2);

            if (!rhs.isImmediate() || rhs.getImmediate() == 0 || rhs.getImmediate() == Long.MIN_VALUE) return false;

            long v2 = rhs.getImmediate();
